    ...
    <div>
      ...
      <div className="flex flex-col gap-3">
        {cats.filter((c) => c.id !== "receita").map((cat) => (
          <div key={cat.id} className="border border-gray-200 rounded-lg p-3 bg-white">
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              {editing === cat.id ? (
                <input
                  ...
                  autoFocus
                  className="text-base font-semibold border border-gray-300 rounded px-2 py-0.5"
                />
              ) : (
                <span className="font-semibold text-base cursor-pointer" onClick={() => setEditing(cat.id)}>
                  {cat.name}
                </span>
              )}
              <span className="text-gray-400 text-xs">({cat.keywords.length} keywords)</span>
              <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${cat.type === "credit" ? "text-green-700 bg-green-100" : "text-red-700 bg-red-100"}`}>
              ...
              {cat.id !== "outros" && (
                <>
                  ...
                  <button onClick={() => handleDelete(cat.id)} className="ml-auto text-red-600 text-xs font-semibold cursor-pointer hover:text-red-800 transition-colors">
                    Deletar
                  </button>
                </>
              )}
            ...
            <div className="flex flex-wrap gap-1">
              {cat.keywords.map((kw) => (
                <span key={kw} className="inline-flex items-center gap-1 bg-gray-100 px-2 py-0.5 rounded-full text-xs">
                  {kw}
                  {cat.id !== "outros" && (
                    <button onClick={() => handleRemoveKeyword(cat.id, kw)} className="border-none bg-none cursor-pointer text-gray-400 hover:text-gray-600 text-sm p-0 leading-none">
                      ×
                    </button>
                  )}
                </span>
              ))}
              <button onClick={() => handleAddKeyword(cat.id)} className="border border-dashed border-gray-300 rounded-full px-2 py-0.5 text-xs cursor-pointer hover:bg-gray-50 transition-colors">
                +
              </button>
            </div>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              <div>
                <label className="text-xs text-gray-500 font-semibold block mb-1">ID</label>
                <input
                  value={form.id}
              ...
              <div>
                <label className="text-xs text-gray-500 font-semibold block mb-1">Nome</label>
                <input
                  value={form.name}
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="text-xs text-gray-500 font-semibold block mb-1">Cor</label>
                  <input
                    type="color"
                    value={form.color}
                    onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))}
                    className="w-full h-10 border border-gray-300 rounded-md cursor-pointer p-0.5"
                  />
                </div>
                <div className="flex-1">
                  <label className="text-xs text-gray-500 font-semibold block mb-1">Tipo</label>
                  <select
                    value={form.type}
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-md text-sm cursor-pointer hover:bg-gray-50 transition-colors">
                  Cancelar
                </button>
